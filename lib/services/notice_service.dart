...
class NoticeArchiveItem {
  ...
  factory NoticeArchiveItem.fromJson(Map<String, dynamic> j) => NoticeArchiveItem(
    id:             j['id'] as int,
...
class NoticeService {
  ...
  Future<List<NoticeArchiveItem>> getMyArchive(int userId) async {
    try {
      final token = await _token();
      final res = await http.get(
        Uri.parse('$_base/notices/my-archive?userId=$userId'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type':  'application/json',
        },
      ...
      if (res.statusCode == 200) {
        final List<dynamic> data = jsonDecode(res.body);
        return data.map((e) => NoticeArchiveItem.fromJson(e)).toList();
      }
    } catch (_) {}
    return [];
  }
